...
function parseArgs(argv: string[]): Flags {
  ...
  if (positional.length > 0) flags.root = positional[0];
  return flags;
}
    ...
    try {
      ...
      results.push(
        skip(
          ...
          'sensitivity',
          `${(err as Error).message} Nothing mechanically stops a loop from editing a sensitive path.`,
        ),
      );
    }
  }
    ...
    if (!flags.verifierCmd) {
      results.push(
        skip(
          ...
          'Verifier Theater',
          'sensitivity',
          'No --verifier-cmd given. loop-audit scores a verifier by filename alone; without a command to run, this drill cannot tell a real verifier from an empty file.',
        ),
      );
    } else {
...
main().catch((err) => {
  console.error(`loop-drill failed: ${(err as Error).message}`);
  process.exit(2);
});
