...
预期：总行数在 25–40 之间（口径：按 `` `n `` 切分、**含末尾空段**，故比 `ReadAllLines` 多 1）；**8 个 BAD 全部为 `False`**；**6 个 GOOD 全部为 `True`**。
...
Write-Output "前3字节: $($b -join ',') （正确值为 35,32,229 = '#'、' '、'工'；无 BOM 的特征是首字节不是 239）"
$raw=[System.IO.File]::ReadAllText($p,[System.Text.Encoding]::UTF8)
Write-Output "中文完好: $($raw -match '结论跟着证据走')"
```

预期：前 3 字节为 `35,32,229`（**R-5 修正**：原预期写 `35,35,32` 是笔误 —— 那是 `#`+空格+空格，任何 markdown 的首行第三字节都是正文字符，不可能又是空格）；中文完好为 `True`。
...
> **执行偏差记录（N-10）：** 第三轮为满足 `wayfinder` 技能本体的「按名字称呼」与 `wayfinder:map` 要求在交付文件里追加了「按名字称呼（Local Markdown 落地）」整节（`issue-tracker.md` 由 30 行增至 39 行）。本节已同步进上面的步骤 2 内容块。**第三轮当时漏了这一步同步**，导致计划与交付物一度相差 9 行 —— 第四轮评审（R-1，high）抓出后补齐。教训：改交付物必须同时改计划里对应的内容块，否则计划的「替换为以下内容」不再可复现。
...
- 五个标签串：任务 5 的映射表与 `triage-labels.md` 逐字一致；交付的 `issue-tracker.md` **只指向它、不再枚举**（见 N-1b）—— 由任务 5 步骤 3 断言。
...
| 风险 | 缓解 |
| --- | --- |
| 新 `AGENTS.md` 未生效（技能按仓根读取，而 `D:\桌面\1` 不是仓） | 已知限制，本次不可解。须在**收尾说明**里写明：**在 `D:\桌面\deepseek` 下开会话才会加载该块** |
| 删除两份细则不可逆 | 任务 1 已逐字留档；两份文件本就在 git 外的 `$DSH_HOME`，无版本控制兜底，故留档是唯一保障 |
| PowerShell 处理中文导致乱码 | 全局约束已禁用 PS 文本读写；本轮已实际踩坑一次并恢复，见全局约束第 3 条 |

**5. 执行强度判定**（迁移前的升级判据；记为用户裁决：留轻模式 + 只读子代理评审）
